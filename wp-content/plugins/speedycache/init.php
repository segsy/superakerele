...
define('SPEEDYCACHE_PRO_DIR', SPEEDYCACHE_DIR .'/main/premium');
...
define('SPEEDYCACHE_VERSION', '1.0.8');
...
function speedycache_autoloader($class){
	...
	// For Pro
	if(file_exists(SPEEDYCACHE_PRO_DIR.'/'.strtolower($m[1]).'.php')){
		include_once(SPEEDYCACHE_PRO_DIR.'/'.strtolower($m[1]).'.php');
	}
}
...
function speedycache_load_plugin(){
	...
	if(!is_admin()){
		...
		// Image URL rewrite	
		if(defined('SPEEDYCACHE_PRO') && file_exists(SPEEDYCACHE_PRO_DIR . '/image.php') && !empty($speedycache->image['settings']['url_rewrite'])){
...
if(defined('WP_CLI') && !empty(WP_CLI) && defined('SPEEDYCACHE_PRO')){
	include_once SPEEDYCACHE_DIR.'/main/cli.php';
}
