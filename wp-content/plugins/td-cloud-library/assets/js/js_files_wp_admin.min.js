...
if(window.tdbVue&&"undefined"!==typeof window.tdbVue.router){var eventHub=new CloudLibraryVue;CloudLibraryVue.component("image-links",{template:'\n            <a :class="classes" :href="link">{{text}}</a>\n        ',props:{classes:[],link:"",text:""}});window.eventHub.$on("refresh-global-colors",function(){jQuery.ajax({url:td_ajax_url,type:"post",data:{action:"tdc_wm_global_colors_update",td_magic_token:tdc_wm_global_colors_nonce},success:function(a){a=JSON.parse(a);window.tdc_wm_global_colors=a.tdc_wm_global_colors;
var b=":root {";Object.entries(a.tdc_wm_global_colors).forEach(function(a){b+="--"+a[0].replace(/_/g,"-")+":"+a[1].color+";"});b+="}";a=jQuery("style#tdb-global-colors");a.length&&a.html(b);a=window.document.getElementById("tdc-live-iframe");null!==a&&(a=jQuery("style#tdb-global-colors",a.contentWindow.document),a.length&&a.html(b))},error:function(a,b,c){console.log("%c"+c,"color: #dc2121c7")}})});new CloudLibraryVue({el:"#tdb-modal",router:window.tdbVue.router})}
...
(function(a,b){tdbCloudTemplates={init:function(){window.tdbCloudTemplatesVue&&"undefined"!==typeof window.tdbCloudTemplatesVue.router&&a("#tdb-cloud-templates").length&&new CloudLibraryVue({el:"#tdb-cloud-templates",router:window.tdbCloudTemplatesVue.router})}};tdbCloudTemplates.init()})(jQuery,_);
